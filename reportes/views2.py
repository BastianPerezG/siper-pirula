...
class ReporteVentasView(LoginRequiredMixin, TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        # ----------------- Pedidos en Curso (KPIs Extras) -----------------
        pedidos_pendientes_qs = Pedido.objects.filter(
            negocio=negocio
        ).exclude(
            estado_preparacion__in=[Pedido.PREP_RETIRADO, Pedido.PREP_CANCELADO, Pedido.PREP_NO_RETIRA]
        )
        
        cant_pedidos_pendientes = pedidos_pendientes_qs.count()
        monto_pedidos_pendientes = pedidos_pendientes_qs.aggregate(
        ...
        context.update({
            ...
            # Pedidos pendientes
            "cant_pedidos_pendientes": cant_pedidos_pendientes,
            "monto_pedidos_pendientes": monto_pedidos_pendientes,
            "pedidos_pendientes_lista": pedidos_pendientes_qs.order_by("-fecha")[:10],
